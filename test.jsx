...
useEffect(() => {
 ...
 useGSAP(() => {
  ...
  useGSAP(() => {
    gsap.set(vehicalPanelRef.current, {
      y: "100vh",
      opacity: 0,
      display: "none",
    });
    gsap.set(searchingPanelRef.current, {
      y: "100vh",
      opacity: 0,
      display: "none",
    });
    gsap.set(vehicleSearchRef.current, {
      y: "100vh",
      opacity: 0,
      display: "none",
    });
    gsap.set(driverWaitingRef.current, {
      y: "100vh",
      opacity: 0,
      display: "none",
    });
  ...
  useGSAP(
    () => {
      console.log("Vehicle panel animation triggered, isVehicalPanelOpen:", isVehicalPanelOpen);
      slideAnim(isVehicalPanelOpen, vehicalPanelRef);
    },
    [isVehicalPanelOpen],
  );
  useGSAP(
    () => slideAnim(isSearchingPanelOpen, searchingPanelRef),
    [isSearchingPanelOpen],
  );
  useGSAP(
    () => slideAnim(isVehicleSearchOpen, vehicleSearchRef),
    [isVehicleSearchOpen],
  );
  useGSAP(
    () => {
      console.log("Driver waiting panel animation triggered, isDriverWaitingOpen:", isDriverWaitingOpen);
      slideAnim(isDriverWaitingOpen, driverWaitingRef);
    },
    [isDriverWaitingOpen],
  );
  ...
  return (
    <div className="h-screen bg-white flex flex-col relative overflow-hidden font-sans">
      ...
      {/* Side Panels - Shared between Mobile (Bottom Sheet) and Desktop (Side Drawer matching left pane width) */}

      <div
        ref={vehicalPanelRef}
        className={`${isVehicalPanelOpen ? '' : 'hidden opacity-0 translate-y-full'} fixed z-50 bottom-0 left-0 right-0 md:left-12 md:right-auto md:bottom-auto md:top-[120px] md:h-fit md:max-h-[calc(100vh-140px)] md:w-[450px] bg-white rounded-t-3xl md:rounded-3xl shadow-[0_-5px_30px_rgba(0,0,0,0.2)] md:shadow-2xl overflow-y-auto w-full md:border border-gray-100`}
      ...
      <div
        ref={searchingPanelRef}
        className={`${isSearchingPanelOpen ? '' : 'hidden opacity-0 translate-y-full'} fixed z-50 bottom-0 left-0 right-0 md:left-12 md:right-auto md:bottom-auto md:top-[120px] md:h-fit md:max-h-[calc(100vh-140px)] md:w-[450px] bg-white rounded-t-3xl md:rounded-3xl shadow-[0_-5px_30px_rgba(0,0,0,0.2)] md:shadow-2xl overflow-y-auto w-full md:border border-gray-100`}
      ...
      {/* {isDriverWaitingOpen && ( */}
{/* Always mounted — no conditional wrapper */}
<div
